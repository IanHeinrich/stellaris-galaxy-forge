...
fn prevent_writes_one_statement_for_a_pair_with_no_lane() {
    snapshot("prevent_2_9", open(), Op::PreventLane { a: 2, b: 9 });
}
...
fn unprevent_erases_the_whole_line() {
    snapshot("unprevent_9_1", open(), Op::UnpreventLane { a: 1, b: 9 });
}
...
#[test]
fn the_prevent_ops_undo_and_redo_byte_for_byte() {
    round_trip(open(), Op::PreventLane { a: 2, b: 9 });
    round_trip(open(), Op::UnpreventLane { a: 9, b: 1 });
}
...
#[test]
fn a_save_refuses_both_ops() {
    let mut session = common::open();
    for op in [
        Op::PreventLane { a: 1, b: 2 },
        Op::UnpreventLane { a: 1, b: 2 },
    ] {
