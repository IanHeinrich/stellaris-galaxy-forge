...
import { homeIntro, NO_HOME_BESIDE } from "./MarauderSection";
...
/**
 * Opens the scenario under the Paint a Galaxy layer with `SYSTEM` in `role`, and the fixture
 * systems `others` in the roles given, each with the initializer its role implies.
 */
async function openWith(
  role: MarauderRole | null,
  others: Record<number, MarauderRole> = {},
): Promise<void> {
  const initializerOf = (r: MarauderRole) =>
    "home" in r ? `marauder_${r.home}_1` : `marauder_${r.base}_2`;
  ...
  const put = (id: number, r: MarauderRole | null) =>
    systems.set(id, {
      ...systems.get(id)!,
      marauder: r,
      initializer: r === null ? "basic_init_01" : initializerOf(r),
    });
  put(SYSTEM, role);
  for (const [id, r] of Object.entries(others)) put(Number(id), r);
...
describe("a scenario system's marauder clan", () => {
  it("is shown only under the Paint a Galaxy layer for a marauder system, after the fallen empire zone", async () => {
    ...
    expect(sections(overview())).not.toContain("Marauder clan 2");
  });
  ...
  it("explains a home, lists the bases beside it, and offers the clan select and removal", async () => {
    await openWith({ home: 1 }, { 0: { base: 1 }, 2: { base: 1 }, 3: { base: 2 } });
    const html = overview();
    for (const line of homeIntro(1)) expect(html).toContain(escaped(line));
    expect(html).toContain("Raid bases: Sol, Barnard.");
  ...
  it("says nothing of raid bases when none neighbours the home", async () => {
    await openWith({ home: 1 }, { 5: { base: 1 } });
    expect(overview()).not.toContain("Raid bases:");
  });
  ...
  it("names a base's home across its lane, or warns that nothing spawns without one", async () => {
    ...
    expect(html).toContain("Its clan home is Sol.");
